...
QrCode QrCode::encodeSegments(const vector<QrSegment> &segs, Ecc ecl,
    ...
    // ⭐ Correction ici : *8 au lieu de l'emoji
    ...
    bb.appendBits(0, std::min(4,
                              static_cast<int>(dataCapacityBits - bb.size())));
    bb.appendBits(0,
                  (8 - static_cast<int>(bb.size() % 8)) % 8);
    assert(bb.size() % 8 == 0);
...
QrCode::QrCode(int ver, Ecc ecl,
               const vector<uint8_t> &dataCodewords, int msk)
    : version(ver),
    ...
    for (size_t i = 0; i < numAlign; i++) {
        for (size_t j = 0; j < numAlign; j++) {
            if (!((i == 0 && j == 0) ||
                  (i == 0 && j == numAlign - 1) ||
                  (i == numAlign - 1 && j == 0)))
                drawAlignmentPattern(alignPatPos.at(i),
                                     alignPatPos.at(j));
        }
    }
...
bool QrCode::module(int x, int y) const {
    return modules.at(static_cast<size_t>(y))
    .at(static_cast<size_t>(x));
}

vector<uint8_t> QrCode::addEccAndInterleave(
    ...
    int numBlocks =
        NUM_ERROR_CORRECTION_BLOCKS[static_cast<int>(errorCorrectionLevel)][version];
    int blockEccLen =
    ...
    for (int i = 0, k = 0; i < numBlocks; i++) {
        ...
        const vector<uint8_t> ecc =
            reedSolomonComputeRemainder(dat, rsDiv);
        if (i < numShortBlocks)
            dat.push_back(0);
...
long QrCode::getPenaltyScore() const {
    ...
    for (int y = 0; y < size; y++) {
        ...
        result += finderPenaltyTerminateAndCount(
                      runColor, runX, runHistory)
                  * PENALTY_N3;
    }
    ...
    int total = size * size;
    int k = static_cast<int>((std::abs(dark * 20L - total * 10L) +
                              total - 1) / total) - 1;
    assert(0 <= k && k <= 9);
    result += k * PENALTY_N4;
    ...
    if (version == 1)
        ...
        int step = (version == 32) ? 26 :
                       (version * 4 + numAlign * 2 + 1) /
                           (numAlign * 2 - 2) * 2;
        vector<int> result;
        for (int i = 0, pos = size - 7;
             i < numAlign - 1; i++, pos -= step)
...
vector<uint8_t> QrCode::reedSolomonComputeDivisor(int degree) {
    ...
    for (int i = 0; i < degree; i++) {
        for (size_t j = 0; j < result.size(); j++) {
            result.at(j) =
                reedSolomonMultiply(result.at(j), root);
            if (j + 1 < result.size())
...
vector<uint8_t> QrCode::reedSolomonComputeRemainder(
    ...
    for (uint8_t b : data) {
        ...
        for (size_t i = 0; i < result.size(); i++)
            result.at(i) ^=
                reedSolomonMultiply(divisor.at(i), factor);
    }
    return result;
}
...
int QrCode::finderPenaltyCountPatterns(
    ...
    bool core =
        ...
        runHistory.at(4) == n &&
        runHistory.at(5) == n;
    return (core && runHistory.at(0) >= n * 4 &&
                    runHistory.at(6) >= n ? 1 : 0)
           + (core && runHistory.at(6) >= n * 4 &&
                      runHistory.at(0) >= n ? 1 : 0);
}
...
void QrCode::finderPenaltyAddHistory(
    ...
    if (runHistory.at(0) == 0)
        currentRunLength += size;
    std::copy_backward(runHistory.cbegin(),
                       runHistory.cend() - 1,
                       runHistory.end());
